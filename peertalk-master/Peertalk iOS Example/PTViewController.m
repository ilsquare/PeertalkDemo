...
- (void)sendDeviceInfo;
@property (nonatomic, strong) OpenGLView20 *panoplayer;
@end
...
- (void)openglplay
{
    ...
    _panoplayer.backgroundColor = [UIColor blackColor];
    _panoplayer.frame = CGRectMake(0, 64, [UIScreen mainScreen].bounds.size.width, [UIScreen mainScreen].bounds.size.height);
...
- (void)ioFrameChannel:(PTChannel*)channel didReceiveFrameOfType:(uint32_t)type tag:(uint32_t)tag payload:(PTData*)payload {
    ...
    if (type == PTExampleFrameTypeTextMessage) {
        ...
        if (textFrame->length == 960*540*1.5) {
            [self.panoplayer displayYUV420pHardData:(void *)textFrame->utf8text width:960 height:540];
            
        }
        //    [self appendOutputMessage:[NSString stringWithFormat:@"[%@]: %s", channel.userInfo, message]];
